...
<a href="index.php?subtopic=adminpanel&action=install_vocations" target="step" onClick="alert(\'Here you can configure vocations to page >Create Character<. Remember: ALWAYS check >Add to create character list?< near vocations that you want to be available when user open >Create Character< page! \');">8. Configure vocations</a><br>
<a href="index.php?subtopic=adminpanel&action=editaccountmanager" target="step" onClick="alert(\'>accept new e-mail time< can be like 0.001 or 0.05 it mean player can accept new e-mail after >0.001 day<, day = 86400 seconds, 86400*0.001 = 86.4 seconds\');">Configure <b>Account Manager</b></a><br>
<a href="index.php?subtopic=adminpanel&action=editcreateaccount" target="step">Configure <b>Create Account</b></a><br>
<a href="index.php?subtopic=adminpanel&action=editmainconfig" target="step">Configure <b>SITE options</b></a><br>';
...
echo 'Installer try to add new tables and columns to database.<br>';
		...
		elseif($config['server']['sqlType'] == "mysql")
		{
			...
			echo "Added columns to table <b>guilds</b>.<br/>";
			try { $SQL->query("ALTER TABLE `players` ADD `online` INT( 11 ) NOT NULL;"); } catch(PDOException $error) {}
...
$SQL->query('INSERT INTO z_news_tickers (date, author, image_id, text, hide_ticker) VALUES ('.time().', 1, 1, "Gesior ACC - Znote prefix Edition installed!", 0)');
...
$SQL->query('INSERT INTO z_news_big (hide_news, date, author, author_id, image_id, topic, text) VALUES (0, '.time().', "Znote", 1, 0, "Znote prefix Edition!", "Gesior ACC - Znote prefix Edition installed. All options should work fine. Report bugs from the thread on www.otland.net ");');
