...
foreach($SQL->query('SELECT * FROM `guild_wars` WHERE `status` IN (1,4) OR ((`end` >= (UNIX_TIMESTAMP() - 604800) OR `end` = 0) AND `status` IN (0,5));') as $war)
{
		...
		<td colspan='3'><table border=0 cellspacing=1 cellpadding=4 width=100%><tr style='background: " . $config['site']['vdarkborder'] . "'><td>Data</td><td>Result</td><td>Kill</td></tr>";
        if(in_array($war['status'], array(1,4,5)))
        {
                ...
                if(!empty($deaths))
                {
                        foreach($deaths as $death)
                        {
                                ...
                                $others = false;
                                $main_content .= "<tr style='display: none; background: " . (is_int($count / 2) ? $config['site']['darkborder'] : $config['site']['lightborder']) . ";'><td>" . date("j M Y, H:i", $death['date']) . "</td><td><span style='font-weight: bold; color: " . ($death['enemy'] == $war['guild_id'] ? "red" : "lime") . ";'>+</span></td>
								<td><a href='/characters/" . urlencode($death['name']) . "'><b>".$death['name']."</b></a> ";
                        ...
                        $main_content .= "<tr style='display: none; background: " . (is_int($count / 2) ? $config['site']['darkborder'] : $config['site']['lightborder']) . ";'><td colspan='3'><center>There were no frags on this war so far.</center></td></tr>";
        }
        else
                $main_content .= "<tr style='display: none; background: " . (is_int($count / 2) ? $config['site']['darkborder'] : $config['site']['lightborder']) . ";'><td colspan='3'><center>This war did not began yet.</center></td></tr>";
