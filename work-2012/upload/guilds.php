...
if($action == '')
{
	...
	if($logged)
		$main_content .= '<TABLE BORDER=0 WIDTH=100%><TR><TD ALIGN=center><IMG SRC="'.$layout_name.'/images/general/blank.gif" WIDTH=80 HEIGHT=1 BORDER=0<BR></TD><TD ALIGN=center><TABLE BORDER=0 CELLSPACING=0 CELLPADDING=0><FORM ACTION="index.php?subtopic=guilds&action=createguild" METHOD=post><TR><TD>
			...
			<BR /><a href="index.php?subtopic=guilds&action=cleanup_players">Cleanup players</a> - can\'t join guild/be invited? Can\'t create guild? Try cleanup players.
			<BR /><a href="index.php?subtopic=guilds&action=cleanup_guilds">Cleanup guilds</a> - made guild, you are a leader, but you are not on players list? Cleanup guilds!';
	else
		$main_content .= 'Before you can create guild you must login.<br><TABLE BORDER=0 WIDTH=100%><TR><TD ALIGN=center><IMG SRC="'.$layout_name.'/images/general/blank.gif" WIDTH=80 HEIGHT=1 BORDER=0<BR></TD><TD ALIGN=center><TABLE BORDER=0 CELLSPACING=0 CELLPADDING=0><FORM ACTION="index.php?subtopic=guilds&action=login&redirect=guilds" METHOD=post><TR><TD>
...
if($action == 'changerank')
{
	...
	else
	{
		...
		if($guild_vice)
		{
			...
			$main_content .= '<FORM ACTION="index.php?subtopic=guilds&action=changerank&guild='.$guild->getName().'&todo=save" METHOD=post>
				<TABLE BORDER=0 CELLSPACING=1 CELLPADDING=4 WIDTH=100%>
			...
			$main_content .= '</SELECT>&nbsp;&nbsp;&nbsp;<INPUT TYPE=image NAME="Submit" ALT="Submit" SRC="'.$layout_name.'/images/buttons/sbutton_submit.gif" BORDER=0 WIDTH=120 HEIGHT=18></TD><TR>
				</TABLE></FORM><TABLE BORDER=0 CELLSPACING=0 CELLPADDING=0 WIDTH=100%><FORM ACTION="index.php?subtopic=guilds&action=show&guild='.$guild->getName().'" METHOD=post><TR><TD><center><INPUT TYPE=image NAME="Back" ALT="Back" SRC="'.$layout_name.'/images/buttons/sbutton_back.gif" BORDER=0 WIDTH=120 HEIGHT=18></center></TD></TR></FORM></TABLE>';
		}
		else
	...
	if(empty($guild_errors)) 
	{
		...
		foreach($account_players as $player) 
		{
			...
			if(empty($player_rank))
			{
				...
				{
					if($config['site']['guild_need_pacc'] == 'no' || $account_logged->getCustomField("premdays") > 0) 
					{
		...
		if($logged) 
		{
			...
			if($guild_leader) 
			{
				...
				foreach($rank_list as $rank) 
				{
					...
					$main_content .= '<tr bgcolor="'.$bgcolor.'"><td align="center">'.$rank->getId().' // <a href="index.php?subtopic=guilds&guild='.$guild->getName().'&action=deleterank&rankid='.$rank->getId().'" border="0"><img src="http://i63.photobucket.com/albums/h122/Mister_Dude/delete.png" border="0" alt="Delete Rank"></a></td><td><input type="text" name="'.$rank->getId().'_name" value="'.$rank->getName().'" size="35"></td><td align="center"><input type="radio" name="'.$rank->getId().'_level" value="3"';
					if($rank->getLevel() == 3) 
					{
	...
	if(empty($guild_errors)) 
	{
		...
		if(empty($guild_errors)) 
		{
			...
			if($guild_leader) 
			{
				...
				$new_rank->save();
				header("Location: index.php?subtopic=guilds&guild=".$guild->getName()."&action=manager");
	...
	if(empty($guild_errors)) 
	{
		if($logged) 
		{
			...
			if($guild_leader) 
			{
				...
				$main_content .= '<form enctype="multipart/form-data" action="index.php?subtopic=guilds&guild='.$guild->getName().'&action=changedescription" method="POST">
				<input type="hidden" name="todo" value="save" />
		...
		if($logged) 
		{
			...
			if($guild_leader) 
			{
				...
				else
				{
					...
					<form action="index.php?subtopic=guilds&guild='.$guild->getName().'&action=passleadership" METHOD=post><input type="hidden" name="todo" value="save"><input type="text" size="40" name="player"><input type="submit" value="Save"></form>
		...
		if($logged) 
		{
			...
			if($guild_leader) 
			{
				...
				else
				{
					...
					<form action="index.php?subtopic=guilds&guild='.$guild->getName().'&action=deleteguild" METHOD=post><input type="hidden" name="todo" value="save"><input type="submit" value="Yes, delete"></form>
					</td></tr>          </table>        </div>  </table></div></td></tr>';
					$main_content .= '<br/><center><form action="index.php?subtopic=guilds&guild='.$guild->getName().'&action=manager" METHOD=post><div class="BigButton" style="background-image:url('.$layout_name.'/images/buttons/sbutton.gif)" ><div onMouseOver="MouseOverBigButton(this);" onMouseOut="MouseOutBigButton(this);" ><div class="BigButtonOver" style="background-image:url('.$layout_name.'/images/buttons/sbutton_over.gif);" ></div><input class="ButtonText" type="image" name="Back" alt="Back" src="'.$layout_name.'/images/buttons/_sbutton_back.gif" ></div></div></form></center>';
				}
			}
	...
	if(empty($guild_errors)) 
	{
		if($logged) 
		{
			if($group_id_of_acc_logged >= $config['site']['access_admin_panel']) 
			{
				...
				{
					$main_content .= '<div class="TableContainer" >  <table class="Table1" cellpadding="0" cellspacing="0" >    <div class="CaptionContainer" >      <div class="CaptionInnerContainer" >        <span class="CaptionEdgeLeftTop" style="background-image:url('.$layout_name.'/images/content/box-frame-edge.gif);" /></span>        <span class="CaptionEdgeRightTop" style="background-image:url('.$layout_name.'/images/content/box-frame-edge.gif);" /></span>        <span class="CaptionBorderTop" style="background-image:url('.$layout_name.'/images/content/table-headline-border.gif);" ></span>        <span class="CaptionVerticalLeft" style="background-image:url('.$layout_name.'/images/content/box-frame-vertical.gif);" /></span>        <div class="Text" >Guild Deleted</div>        <span class="CaptionVerticalRight" style="background-image:url('.$layout_name.'/images/content/box-frame-vertical.gif);" /></span>        <span class="CaptionBorderBottom" style="background-image:url('.$layout_name.'/images/content/table-headline-border.gif);" ></span>        <span class="CaptionEdgeLeftBottom" style="background-image:url('.$layout_name.'/images/content/box-frame-edge.gif);" /></span>        <span class="CaptionEdgeRightBottom" style="background-image:url('.$layout_name.'/images/content/box-frame-edge.gif);" /></span>      </div>    </div>    <tr>      <td>        <div class="InnerTableContainer" >          <table style="width:100%;" ><tr><td>Are you sure you want delete guild <b>'.$guild_name.'</b>?<br>
					<form action="index.php?subtopic=guilds&guild='.$guild->getName().'&action=deletebyadmin" METHOD=post><input type="hidden" name="todo" value="save"><input type="submit" value="Yes, delete"></form>
					</td></tr>          </table>        </div>  </table></div></td></tr>';
					$main_content .= '<br/><center><form action="index.php?subtopic=guilds" METHOD=post><div class="BigButton" style="background-image:url('.$layout_name.'/images/buttons/sbutton.gif)" ><div onMouseOver="MouseOverBigButton(this);" onMouseOut="MouseOutBigButton(this);" ><div class="BigButtonOver" style="background-image:url('.$layout_name.'/images/buttons/sbutton_over.gif);" ></div><input class="ButtonText" type="image" name="Back" alt="Back" src="'.$layout_name.'/images/buttons/_sbutton_back.gif" ></div></div></form></center>';
				}
			}
	...
	if(empty($guild_errors)) 
	{
		if($logged) 
		{
			...
			if($guild_leader) 
			{
				...
				$main_content .= '<form enctype="multipart/form-data" action="index.php?subtopic=guilds&guild='.$guild->getName().'&action=changemotd" method="POST">
				<input type="hidden" name="todo" value="save" />
		...
		if($logged) 
		{
			...
			if($guild_leader) 
			{
				...
				if(!empty($ranks_errors)) 
				{
					...
					header("Location: index.php?subtopic=guilds&action=manager&guild=".$guild->getName());
			}
			else
				$guild_errors[] = 'You are not a leader of guild!';
		}
